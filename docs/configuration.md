...
[`docs/state-inventory.md`](state-inventory.md) is the single owner of the exhaustive per-file inventory: use it to identify a file you have found in an operational home and learn which script owns it.
...
Bootstrap also verifies the lint gate's pinned ShellCheck through `bin/fm-lint.sh --ensure-shellcheck`, so a home that cannot run the pre-push lint learns it at session start instead of at push time, when a refusal has already stopped work.
An unresolvable pin reports a `MISSING: shellcheck` line whose install command is `bin/fm-lint.sh --ensure-shellcheck`, so it means the pinned build is missing rather than `shellcheck` itself, and the approved install fills `fm-lint.sh`'s private version-keyed cache without replacing the machine's own `shellcheck`; [`CONTRIBUTING.md`](../CONTRIBUTING.md) owns that resolution contract.
