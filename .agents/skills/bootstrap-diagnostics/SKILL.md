...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `shellcheck`, this means the lint gate's pinned build could not be resolved, not that the machine has no `shellcheck`; translate it as readying the pre-push lint gate, because the approved install only fills `bin/fm-lint.sh`'s private cache and never touches the captain's own `shellcheck`.
